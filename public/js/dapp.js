...
if (window.ethereum != null) {
	ethereum.on("chainChanged", chainId => {
		if (chainId != 4) {
			removeGridsUntilConnected(
				"You must be connected to the Sepolia network to use Balloon",
				"network"
			);
		} else {
...
async function layout(perPage, page) {
	...
	if (length == 0) {
		...
		depositNFTButton.onclick = function () {
			getAccount().then(() => {
				location.href = "/profile/?func=deposit";
			});
		};
	...
	for (stIndex; stIndex < inIndex + length; stIndex++) {
		...
		clone.getElementsByClassName("priceLabel")[0].innerHTML = escapeHTML(
			valuation.div(1000000000000000000).toString()
		);
		clone.getElementsByClassName("collateralLabel")[0].innerHTML =
			"@ " + escapeHTML(collateralMultiplier.div(100).toString() + "x");
		clone.getElementsByClassName("nftTokenId")[0].innerHTML =
			"#" + escapeHTML(currentNFT.tokenId.toString());
		clone.getElementsByClassName("nftCollectionName")[0].innerHTML = escapeHTML(
			truncaddy(currentNFT.addy, 10)
		);

		clone.addEventListener(
			"click",
			function () {
				redirectToBorrowable(currentNFT.addy, currentNFT.tokenId);
			},
			false
		);

		borrowableGrid.insertBefore(
			clone,
			borrowableGrid.childNodes[length - (stIndex - inIndex)]
		);
	}
...
function removeGridsUntilConnected(message, which, linkref) {
	...
	if (which != null) {
		if (which == "get") {
			...
			connectButton.innerHTML = "Switch to Sepolia";
			connectButton.classList.add("providerErrorConnectButton");
			metaNotifierWrapper.appendChild(connectButton);
		}
	}
...
async function checkConnection() {
	if (window.ethereum == null) {
		removeGridsUntilConnected(
			"You must install Metamask to use Balloon.",
			"get"
		);
		return;
	}
	provider.listAccounts().then(result => {
		if (result.length == 0) {
			removeGridsUntilConnected(
				"Please connect to Metamask to use Balloon",
				"connect"
			);
			return;
		}
